...
namespace MatrixEngine.Core.IntegrationTest.Tests.case_15_same_block_events;

[Order(3)]
public class TestCase15 : TestBed<IntegrationTestFixture>
{
    public TestCase15(ITestOutputHelper testOutputHelper, IntegrationTestFixture fixture) : base(testOutputHelper,
        fixture)
    {
    }
    ...
    {
        // This test case will test what happens if a user has multiple events in the same block.
        // In this test, the user bonds, unbonds, withdraws and rebonds all in block 0
        // They also bond and unbond in block 900, which is the first block of the second cycle. 
        // This second part is important as the system will automatically fill out their previous balance
        // as a fake balance entry for block 900

        var dataLoader = _fixture.GetService<IDataLoader>(_testOutputHelper);
        await dataLoader?.LoadCase("case-15")!;
        ...
        var balanceChanges = database.GetCollection<BalanceModel>(DbCollectionName.Balance);
