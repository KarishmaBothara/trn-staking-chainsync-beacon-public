...
public class BalanceChangeResolverTests
{
    ...
    {
        var transactions =
            JsonFileReader.Read<List<TransactionModel>>(@"./Data/transactions/single-account-bonded-balance-snapshot.json");
        ...
        var balanceChanges = balanceChange[account];
        Assert.Equal(4, balanceChanges.Count);

        Assert.Equal(0, balanceChanges[0].StartBlock);
        Assert.Equal(9, balanceChanges[0].EndBlock);
        Assert.Equal(0, balanceChanges[0].PreviousBalance); //balance snapshot
        Assert.Equal(100, balanceChanges[0].BalanceChange);
        Assert.Equal(100, balanceChanges[0].BalanceInBlockRange);

        Assert.Equal(10, balanceChanges[1].StartBlock);
        Assert.Equal(49, balanceChanges[1].EndBlock);
        Assert.Equal(100, balanceChanges[1].PreviousBalance);
        Assert.Equal(100, balanceChanges[1].BalanceChange);
        Assert.Equal(200, balanceChanges[1].BalanceInBlockRange);

        Assert.Equal(50, balanceChanges[2].StartBlock);
        Assert.Equal(149, balanceChanges[2].EndBlock);
        Assert.Equal(200, balanceChanges[2].PreviousBalance);
        Assert.Equal(100, balanceChanges[2].BalanceChange);
        Assert.Equal(300, balanceChanges[2].BalanceInBlockRange);
        
        Assert.Equal(150, balanceChanges[3].StartBlock);
        Assert.Equal(200, balanceChanges[3].EndBlock);
        Assert.Equal(300, balanceChanges[3].PreviousBalance);
        Assert.Equal(100, balanceChanges[3].BalanceChange);
        Assert.Equal(400, balanceChanges[3].BalanceInBlockRange);
    }
    ...
    public async Task CalculateBalanceChanges_WhenSingleAccount_BondedAndWithdrawn()
    {
        ...
        Assert.Equal(49, balanceChanges[0].EndBlock);
        Assert.Equal(0, balanceChanges[0].PreviousBalance);
        Assert.Equal(100, balanceChanges[0].BalanceChange); //balance snapshot
        Assert.Equal(100, balanceChanges[0].BalanceInBlockRange);
    ...
    {
        var transactions =
            JsonFileReader.Read<List<TransactionModel>>(@"./Data/transactions/single-account-bonded-withdrawn-balance-snapshot.json");
        var balanceSnapshots = new List<BalanceSnapshotModel>
        {
            new()
            {
        ...
        var balanceChanges = balanceChange[account];
        Assert.Equal(6, balanceChanges.Count);
        ...
        Assert.Equal(150, balanceChanges[3].StartBlock);
        Assert.Equal(169, balanceChanges[3].EndBlock);
        Assert.Equal(300, balanceChanges[3].PreviousBalance);
        Assert.Equal(100, balanceChanges[3].BalanceChange);
        Assert.Equal(400, balanceChanges[3].BalanceInBlockRange);

        Assert.Equal(170, balanceChanges[4].StartBlock);
        Assert.Equal(199, balanceChanges[4].EndBlock);
        Assert.Equal(400, balanceChanges[4].PreviousBalance);
        Assert.Equal(-20, balanceChanges[4].BalanceChange);
        Assert.Equal(380, balanceChanges[4].BalanceInBlockRange);

        Assert.Equal(200, balanceChanges[5].StartBlock);
        Assert.Equal(300, balanceChanges[5].EndBlock);
        Assert.Equal(380, balanceChanges[5].PreviousBalance);
        Assert.Equal(100, balanceChanges[5].BalanceChange);
        Assert.Equal(480, balanceChanges[5].BalanceInBlockRange);
    }
