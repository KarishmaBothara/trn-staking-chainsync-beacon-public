...
namespace MatrixEngine.Core.IntegrationTest.Tests.case_7_ex_4;

[Order(7)]
public class TestCase7 : TestBed<IntegrationTestFixture>
{
    public TestCase7(ITestOutputHelper testOutputHelper, IntegrationTestFixture fixture) : base(testOutputHelper,
        fixture)
    {
    }
    ...
    public async Task Test_Scenario_1()
    {
        ...
        await dataLoader?.LoadCase("case-7")!;
        ...
        Assert.Equal("2071865", balance.TotalRewardPoints);
        ...
        Assert.Equal(134, entry3.EndBlock);
        ...
        Assert.Equal("60000000", entry3.Bonded.EffectiveBalance);
        Assert.Equal("0.0246", entry3.Bonded.Rate.ToString());
        Assert.Equal("Staker", entry3.Bonded.StakerType);
        Assert.Equal("180399", entry3.Bonded.RewardPoints);
        // Unlocking properties
        ...
        Assert.Equal(135, entry4.StartBlock);
        Assert.Equal(449, entry4.EndBlock);
        Assert.Equal(315, entry4.EffectiveBlocks);
        Assert.Equal(1, entry4.VtxDistributionId);
        Assert.Equal(0.35, entry4.Percentage);
        // Bonded properties
        Assert.Equal("60000000", entry4.Bonded.Balance);
        Assert.Equal("60000000", entry4.Bonded.EffectiveBalance);
        Assert.Equal("0.0246", entry4.Bonded.Rate.ToString());
        Assert.Equal("Staker", entry4.Bonded.StakerType);
        Assert.Equal("516600", entry4.Bonded.RewardPoints);
        // Unlocking properties
        ...
        Assert.Equal(450, entry5.EffectiveBlocks);
        Assert.Equal(1, entry5.VtxDistributionId);
        Assert.Equal(0.5, entry5.Percentage);
        // Bonded properties
        Assert.Equal("110000000", entry5.Bonded.Balance);
        ...
        Assert.Equal("1353000", entry5.Bonded.RewardPoints);
        // Unlocking properties
